 ...
 *
 * Basic test for the QMC5883P 3-axis magnetometer, with compass heading
 */
...
// Your magnetic declination in radians (find yours at http://www.magnetic-declination.com/)
// Example: -13* 2' W  ~=  -0.22 radians (adjust sign/value for your location)
  ...
  if (qmc.isDataReady()) {
    ...
    if (qmc.getRawMagnetic(&x, &y, &z)) {
      ...
      if (qmc.getGaussField(&gx, &gy, &gz)) {
        ...
        // --- Heading calculation ---
        // Hold the module level (Z pointing up) for this to be accurate.
        float heading = atan2(gy, gx);
